...
impl ServerFixture {
    ...
    pub async fn create_shared() -> Self {
        ...
        // one present
        static SHARED_SERVERS: OnceCell<parking_lot::Mutex<Option<Weak<TestServer>>>> =
            OnceCell::new();
        let shared_servers = SHARED_SERVERS.get_or_init(|| parking_lot::Mutex::new(None));

        let mut shared_servers = shared_servers.lock();

        // is a shared server already present?
        let server = match shared_servers.map(|s| s.upgrade()).as_ref().as_ref()
        {
            Some(Some(server)) => *server,
            _ => {
                // if not, create one
                ...
                *shared_servers = Some(Arc::downgrade(&server));
                server
            }
        };
...
impl TestServer {
    ...
    fn create_server_process(
        ...
        let child = Command::cargo_bin("ingester_cli")
            .unwrap()
